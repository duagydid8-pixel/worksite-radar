...
window.collectAllPagesFetch=async function(){var ifr=document.querySelector("iframe");if(!ifr)return JSON.stringify({error:"no iframe"});var w=ifr.contentWindow;var grid=w.$find&&w.$find("WebDataGrid1");if(!grid)return JSON.stringify({error:"no grid"});var paging=grid.get_behaviors().get_paging();var totalPages=paging.get_pageCount();window.__items=[];var seen=new Set();function extract(doc){var els=doc.querySelectorAll('[onclick^="popupTimebook"]');var added=0;els.forEach(function(el){var oc=el.getAttribute("onclick");var args=(oc.match(/'([^']*)'/g)||[]).map(function(s){return s.slice(1,-1)});var k=args[0]+"|"+args[1];if(!seen.has(k)){seen.add(k);var tr=el.closest("tr");var tds=tr?Array.from(tr.querySelectorAll("td")).map(function(c){return c.textContent.trim().replace(/\\s+/g," ")}):[];window.__items.push({mpw_idx:args[0],mpw_chk_idx:args[1],mpw_type:args[2]||"S",tds:tds.slice(0,20)});added++}});console.log("p"+(window.__items.length)+"(+"+added+")");return added}function getVS(doc){return{vs:(doc.querySelector('[name="__VIEWSTATE"]')||{value:""}).value,vsg:(doc.querySelector('[name="__VIEWSTATEGENERATOR"]')||{value:""}).value,ev:(doc.querySelector('[name="__EVENTVALIDATION"]')||{value:""}).value}}extract(ifr.contentDocument);if(totalPages<=1)return JSON.stringify({totalPages:totalPages,collected:window.__items.length,method:"fetch+viewstate"});var ifrDoc=ifr.contentDocument;var form=ifrDoc.forms[0];var action=form.action||ifrDoc.location.href;var vs=getVS(ifrDoc);for(var p=1;p<totalPages;p++){var fd=new FormData(form);fd.set("__EVENTTARGET","WebDataGrid1$ctl01$ucPagerControl$btnNext");fd.set("__EVENTARGUMENT","");fd.set("__VIEWSTATE",vs.vs);fd.set("__VIEWSTATEGENERATOR",vs.vsg);fd.set("__EVENTVALIDATION",vs.ev);try{var resp=await fetch(action,{method:"POST",body:fd,credentials:"include"});var html=await resp.text();if(html.length<1000||html.indexOf("Login.aspx")>=0)return JSON.stringify({error:"session expired",collected:window.__items.length,page:p});var doc=new DOMParser().parseFromString(html,"text/html");extract(doc);vs=getVS(doc)}catch(e){console.log("fetch err p"+p+":"+e.message)}await new Promise(function(r){setTimeout(r,300)})}return JSON.stringify({totalPages:totalPages,collected:window.__items.length,method:"fetch+viewstate"})};
window.collectAllPages=async function(){var ifr=document.querySelector("iframe");if(!ifr)return JSON.stringify({error:"no iframe"});var w=ifr.contentWindow;var grid=w.$find&&w.$find("WebDataGrid1");if(!grid)return JSON.stringify({error:"no grid"});var paging=grid.get_behaviors().get_paging();var totalPages=paging.get_pageCount();window.__items=[];var seen=new Set();function extractCurrent(){var d=ifr.contentDocument;var els=d.querySelectorAll('[onclick^="popupTimebook"]');var added=0;els.forEach(function(el){var oc=el.getAttribute("onclick");var args=(oc.match(/'([^']*)'/g)||[]).map(function(s){return s.slice(1,-1)});var k=args[0]+"|"+args[1];if(!seen.has(k)){seen.add(k);var tr=el.closest("tr");var tds=tr?Array.from(tr.querySelectorAll("td")).map(function(c){return c.textContent.trim().replace(/\\s+/g," ")}):[];window.__items.push({mpw_idx:args[0],mpw_chk_idx:args[1],mpw_type:args[2]||"S",tds:tds.slice(0,20)});added++}});console.log("p"+(window.__items.length)+"(+"+added+")");return added}function postback(suffix){var d=ifr.contentDocument;d.querySelector('[name="__EVENTTARGET"]').value="WebDataGrid1$ctl01$ucPagerControl$"+suffix;d.querySelector('[name="__EVENTARGUMENT"]').value="";d.forms[0].submit()}postback("btnFirst");await new Promise(function(r){setTimeout(r,6000)});extractCurrent();for(var p=1;p<totalPages;p++){postback("btnNext");await new Promise(function(r){setTimeout(r,6000)});extractCurrent()}return JSON.stringify({totalPages:totalPages,collected:window.__items.length})};
      ...
      <DialogContent className="max-w-xl max-h-[80vh] overflow-y-auto">
        ...
        <div className="space-y-4 text-sm">
          ...
          <div className="rounded-lg bg-slate-50 border border-slate-200 p-3 space-y-2">
            ...
            <p className="text-xs text-emerald-700 font-bold">✓ "OK v7: collectAllPagesFetch / ..." 메시지 확인</p>
          </div>
          <div className="rounded-lg bg-slate-50 border border-slate-200 p-3 space-y-2">
            <p className="font-black text-slate-800">③ 순서대로 실행</p>
            <div className="space-y-2">
              {[
                { step: "전체 페이지 수집 (fetch방식, 빠름 ~5초)", code: "await collectAllPagesFetch()" },
                { step: "프로젝트 코드 확인", code: "getPjtCd()" },
                { step: "IN/OUT 데이터 수집 (날짜·pjt_cd 자동 감지)", code: "await fetchAllInOut()" },
                { step: "파싱", code: "parseAndJoin()" },
                { step: "CSV 다운로드 (파일명 자동)", code: "downloadCsv()" },
              ].map(({ step, code }, i) => (
